...
Update log (modifications by Justin Chew):

---- January 6, 2016 ----
Added new functions "newRemoveSmallBlobs", "contourTouchesEdge", and
"thresholdFillContours" to replace removeSmallBlobs in the initial
preProcessCyano function so that colonies of cells with higher cell
densities have improved segmentation.

---- November 9, 2016 ----
In the preProcessCyano function, changed the size of the characteristic
"object" to be detected from 10 to 20 pixels, effectively increasing
the thickness of the border around cell colonies. Previously, borders
that were too thin led occasionally to situations where the entire
colony was lost due to there being a small hole connecting the background
with one of the interior gaps between cells.

The relevant line of code below is the changed version:

brightScaled = mat2gray(bpass(brightImg, 1, 20), 255)
...
def contourTouchesEdge(img, contour):
    """
    contourTouchesEdge determines whether a given contour touches the
    edge of an image or not, returning True if it does and False if it
    does not.
    """
    ...
    edge_values = [1, x_dim-1, y_dim-1]
    
    if (np.max(contour) in edge_values) or (np.min(contour) in edge_values):
        return True
    else:
        return False


def thresholdFillContours(input_img, area_threshold, fill_value, overwrite_flag=False, thresh_fill_direction="above", handle_edge_contours=False):
    """
    thresholdFillContours takes an input image and returns an image
    where all the contours with an area above a threshold are filled in
    with the specified value.
    
    overwrite_flag specifies whether the contour filling should return
    a new image with only the filled contours (flag = False) or whether
    it should return the filled contours on top of the original image
    (flag = True).
    
    thresh_fill_direction indicates whether values above or below the
    area size threshold should be filled with the specified value.
    
    handle_edge_contours indicates whether contours that border the edge
    of an image (e.g. a colony of cells) should be processed specially,
    for which it currently fills the bounding box surrounding the edge
    contours (useful for producing a mask image).
    """
    ...
    # fetch contours that are above a certain area threshold
    threshold_contours = []
    for i in range(len(areas)):
        if thresh_fill_direction == "above":
            if areas[i] > area_threshold:
                threshold_contours.append(contours[i])
        if thresh_fill_direction == "below":
            if areas[i] < area_threshold:
                threshold_contours.append(contours[i])

    # create image with contours past threshold with filled interiors
    if overwrite_flag == False:
        img_thresh_contours = np.zeros_like(input_img)
    if overwrite_flag == True:
        img_thresh_contours = input_img.copy()
    for contour in threshold_contours:
        # note: each contour needs to be treated separately in order for
        # all of them to be filled regardless of the contour hierarchy
        cv.drawContours(img_thresh_contours, [contour], 0, fill_value, -1)

    # add on edge contours with bounding boxes filled
    if handle_edge_contours == True:
        for contour in threshold_contours:
            if contourTouchesEdge(img_thresh_contours, contour):
                x, y, w, h = cv.boundingRect(contour)
                cv.rectangle(img_thresh_contours, (x, y), (x+w, y+h), fill_value, -1)
        
    return img_thresh_contours


def newRemoveSmallBlobs(input_img, area_threshold):
    """
    newRemoveSmallBlobs fixes the issue in preProcessCyano where the
    original removeSmallBlobs function not only removed the junk blobs
    that were not cells, but also removed the interior segmentations
    within a colony of cells since the cell walls did not wrap
    completely around each cell.  This should lead to better cell
    segmentation.
    
    Recommended value for the area_threshold is 1000 (the average single
    cell area is around 1300-1500).
    """

    # generate an image with filled in cells as a mask and set everything outside of it to black
    img_mask = thresholdFillContours(input_img, area_threshold, 1, handle_edge_contours=True)
    img_cells_only = img_mask * input_img

    return img_cells_only
...
def preProcessCyano(brightImg, chlorophyllImg=0, mask=False):
    ...
    dilatedIm = newRemoveSmallBlobs(processedBrightfield*cellMask, 1000)
...
#   dilatedIm = (removeSmallBlobs(processedBrightfield*cellMask, 15))
    ...
    # modify imgOut with thresholdFillContours to fill in any gaps within the cells
    imgOut = thresholdFillContours(255-dilatedIm.copy(), 45, 255, True, "below")
...
def findFamilyID(trIn, dim=8):
    ...
    """
    # following line seems to be the cause of a bug, don't know what this line does...
